...
—◉ *في القمة ${len} ذهب *
*👤 موقعك:* ${userscredit.indexOf(m.sender) + 1} من ${userscredit.length}

${sortedcredit.slice(0, len).map(({jid, credit}, i) => `${i + 1}. ${participants.some((p) => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : '@'}${jid.split`@`[0]} *${credit} ذهب*`).join`\n`}
